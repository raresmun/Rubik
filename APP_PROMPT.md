...
## Updated delivery goal (September 2026)

The implementation also targets iPhone/iPad Safari and installed iOS home-screen use, safe areas, portrait/landscape, explicit audio activation and platform-specific Romanian installation instructions. Deliver changes to this existing GitHub repository and deploy publicly to Vercel. A verified HTTPS URL, actual rendered screenshots and a concise evidence-based testing report are required; distinguish real devices, emulation and unit/DOM tests. Keep the premium off-white/navy/blue/teal visual direction and precise interactive 3D cube. The latest user goal is authoritative where it extends this original brief.
